...
|---|---|---|---|
| 表情踏板 1 | 张嘴 | CC11,0–127 连续 | 链上第 1 块摇杆类踏板(Whammy/Wah/Crybaby 的 TREADLE position);无摇杆踏板则不动作 |
| 表情踏板 2 | (motion_midi 第二踏板) | CC12,0–127 连续 | 链上第 2 块摇杆类踏板;没有则不动作 |

表情踏板**故意不做**音量踏板/Master 兜底:表情静止时输出 0,映射到音量类参数
会把输出拉到行程最底(嘴闭=静音,曾因此误判为"没声音")。
