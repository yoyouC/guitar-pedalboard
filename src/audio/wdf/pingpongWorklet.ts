/**
 * 乒乓延迟(Ping-Pong Delay)的 AudioWorklet 处理器(Blob 加载,免构建配置)。
 *
 * DSP 核见同目录 pingPongDelay.dsp.js(链路说明也在那里)——经 `?raw` 取源码
 * 字符串,与下方 wrapper(parameterDescriptors / Processor 子类 /
 * registerProcessor)拼装进 Blob(ADR-0001 语义不变,ADR-0003 双模式消费)。
 */
import { buildProcessorSource, createWorkletLoader } from '../workletLoader';
import pingPongDelaySource from './pingPongDelay.dsp.js?raw';

const processorSource = buildProcessorSource(
  [pingPongDelaySource],
  `
class PingPongDelayProcessor extends AudioWorkletProcessor {
  static get parameterDescriptors() {
    return [
      { name: 'time', defaultValue: 400, minValue: 50, maxValue: 1500 },
      { name: 'feedback', defaultValue: 40, minValue: 0, maxValue: 90 },
      { name: 'mix', defaultValue: 30, minValue: 0, maxValue: 100 },
    ];
  }

  constructor() {
    super();
    this.engine = new PingPongDelayEngine(sampleRate);
  }

  process(inputs, outputs, params) {
    return this.engine.process(inputs, outputs, params);
  }
}

registerProcessor('pingpong-delay', PingPongDelayProcessor);
`,
);

/** 幂等加载(按 AudioContext 注册),使用前必须先 await */
export const loadPingPongDelay = createWorkletLoader(processorSource);
