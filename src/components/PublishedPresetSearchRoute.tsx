        ...
        {([
          ...
          <button
            ...
            onClick={() => {
              setTab(kind);
              setMessage('');
              if (kind === 'presets' && (lastRequest.text ?? '') !== text) {
                void runSearch({ ...lastRequest, text, cursor: undefined }, false);
              }
              if (
                kind === 'collections'
                && (collectionItems.length === 0 || collectionText !== text)
              ) {
                void runCollectionSearch(text, null, false);
              }
              if (kind === 'creators' && (creatorItems.length === 0 || creatorText !== text)) {
                void runCreatorSearch(text, null, false);
              }
            }}
          >{label}</button>
